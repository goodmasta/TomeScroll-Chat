...
- **Discord-style unread tracking**: opening a tab scrolls to a "New messages" divider marking where you left off, rather than jumping straight to the bottom. The divider stays put as you read - it doesn't recompute until you switch away and back.
...
- **Message input** starts one line tall and grows up to 3 as you add line breaks - **Enter sends, Shift+Enter inserts a line break**, Telegram/Discord-style - then shrinks back to one line once sent. Text also **wraps automatically** once a line fills the available width, same as it wrapping while you type in any normal messenger. Has, attached flush to its right edge: a "jump to bottom" button (always visible, only clickable while you're scrolled up from the bottom), a "select text" toggle, and an emote-picker button (searchable image grid: standard emoji, then BTTV/7TV). Pressing Enter with no ImGui text field focused anywhere opens/focuses the current tab's input, mimicking the game's own "press Enter to chat" keybind.
...
- The title bar always matches the window body's own background colour (and fades along with it, see above), rather than following the current theme's accent colour like a separate strip.
