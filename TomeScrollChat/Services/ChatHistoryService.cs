...
public sealed class ChatHistoryService : IDisposable
{
    ...
    public void ClearAll()
    {
        try
        {
            ...
            cmd.ExecuteNonQuery();
            log.Information("TomeScrollChat: cleared all chat history");
        }
        catch (Exception ex)
        {
            log.Error(ex, "TomeScrollChat: failed to clear chat history");
        }
    }
    ...
    private async Task WriterLoopAsync(CancellationToken token)
    {
        ...
        try
        {
            while (await pending.Reader.WaitToReadAsync(token).ConfigureAwait(false))
            {
                ...
                {
                    log.Error(ex, "TomeScrollChat: failed to write {Count} chat message(s) to history", batch.Count);
                }
    ...
    private List<ChatPayloadLink> DeserializePayloadLinks(string? json)
    {
        ...
        {
            log.Warning(ex, "TomeScrollChat: failed to restore stored payload links from history");
            return new List<ChatPayloadLink>();
        }
    }
    ...
    public void SaveTranslation(ChatMessageRecord record, string translatedText, string targetLanguage)
    {
        ...
        {
            log.Warning(ex, "TomeScrollChat: failed to persist a translation to history");
        }
    }
    ...
    public double EstimateAverageBytesPerMessage()
    {
        ...
        {
            log.Warning(ex, "TomeScrollChat: failed to estimate average message size");
            return FallbackAverageBytesPerMessage;
        }
    }
    ...
    public long GetCurrentSizeBytes()
    {
        ...
        {
            log.Warning(ex, "TomeScrollChat: failed to read history database size");
            return 0;
        }
    }
    ...
    private void EnforceSizeCap()
    {
        try
        {
            ...
            log.Information("TomeScrollChat: history size {Size} bytes exceeds cap {Max} bytes, pruning oldest messages", CurrentSize(), maxBytes);
        ...
        {
            log.Error(ex, "TomeScrollChat: failed to enforce history size cap");
        }
    }
