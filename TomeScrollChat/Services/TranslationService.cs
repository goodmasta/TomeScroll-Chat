...
public sealed class TranslationService : IDisposable
{
            ...
            while (await queue.Reader.WaitToReadAsync(token).ConfigureAwait(false))
            {
                while (queue.Reader.TryRead(out var job))
                {
                    ...
                    if (consecutiveFailures > 0)
                    {
                        ...
                        log.Warning("TomeScrollChat: translation failed ({Count} in a row) - backing off {Delay:0}s before the next request", consecutiveFailures, delay.TotalSeconds);
                    }
    ...
    private async Task<string?> TranslateViaGoogleGtxAsync(string text, string targetLanguage)
    {
        ...
        {
            log.Warning(ex, "TomeScrollChat: Google translate failed");
            return null;
        }
    }
    ...
    private async Task<string?> TranslateViaMyMemoryAsync(string text, string targetLanguage)
    {
        ...
        {
            log.Warning(ex, "TomeScrollChat: MyMemory translate failed");
            return null;
        }
    }
    ...
    private string? ParseMyMemoryResponse(string json)
    {
        ...
        {
            log.Warning("TomeScrollChat: MyMemory's free daily quota is exhausted");
            return null;
        }
