...
/// <c>/customchat friends</c> (<see cref="ShowOnScreen"/>) is the only way to bring it back afterward.
...
public sealed unsafe class FriendOnlineWatcherService : IDisposable
{
    ...
    /// <summary>"/customchat friends" - brings the Friend List (and the surrounding Social window it
    ...
    private void OnFrameworkUpdate(IFramework _)
    {
        ...
        if (wasEnabled && !configuration.FriendOnlineNotifyEnabled)
        {
            ...
            {
                log.Warning(ex, "CustomChat: failed to close the Friend List window after disabling the feature");
            }
        }
    ...
    private void OpenAddons()
    {
        try
        {
            ...
            log.Info("CustomChat: opened the Friend List and Social addons");
        }
        catch (Exception ex)
        {
            log.Warning(ex, "CustomChat: failed to open the Friend List window");
        }
    }
    ...
    private void CheckFriends()
    {
        ...
        foreach (var key in watchedKeys)
        {
            ...
            {
                log.Info("CustomChat: friend-watch detected a change for {Key}: {Was} -> {Now}", key, wasOnline, isOnline.Value);
    ...
    /// <summary>"/customchat frienddebug" - lets testing this feature not depend on catching a real
    ...
    public void DebugCheckAndNotify()
    {
        ...
        log.Info("CustomChat: friend list debug dump ({Count} entries)", allFriends.Count);
        foreach (var (key, _) in allFriends)
        {
            ...
            log.Info("CustomChat:   {Key} - {Status}", key, isOnline switch
            {
                true => "online",
                false => "offline",
