...
public sealed unsafe class NativePartyFinderLinkWatcher : IDisposable
{
    ...
    private void OnFrameworkUpdate(IFramework _)
    {
        ...
        log.Info("CustomChat: native party finder relay detected - listing {ListingId} ({LeaderName})", listingId, leaderName);
    ...
    private byte[]? TryCaptureNativePayloadBytes()
    {
        ...
        {
            log.Warning(ex, "CustomChat: failed to capture the native party finder link payload bytes");
            return null;
        }
    }
