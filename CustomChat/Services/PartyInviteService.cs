...
public sealed unsafe class PartyInviteService
{
    ...
    public bool Invite(string name, string world)
    {
        ...
        if (worldId == null)
        {
            log.Warning("CustomChat: couldn't resolve world '{World}' for a party invite to {Name}", world, name);
            return false;
        }
        ...
        {
            log.Warning(ex, "CustomChat: failed to send a party invite to {Name}@{World}", name, world);
            return false;
        }
    }
}
